...
class TwoEchelonPLSTS(gym.Env):
    ...
    def __init__(
        ...
        holding_store=1,
        penalty=18,
        initial_inventory=[[50, 0, 0], [50, 0, 0], [50, 0, 0]],
        online_demand_params=[0 for i in range(100)],
        ...
        if self.demand_distribution is None:
            warnings.warn(
                "You haven't set the demand distribution, so we're assuming Poisson for all stores. And doing some slow manipulation. Please set the demand_distribution parameter."
            )
            self.demand_distribution = ["Poisson" for i in range(self.N + 1)]
            store_demand_params = [
                [store_demand_params[t][i] for t in range(self.periods)]
                for i in range(self.N)
            ]
        else:
            assert len(demand_distribution) == self.N + 1, (
                "You need to specify demand distributions for each store AND the online channel. Even if there's no demand to the online channel"
            )
        ...
        assert self.ts_lt <= self.st_lt, (
            "The transhipment lead time cannot be more than the warehouse lead-time"
        )
        assert self.cap_prod >= 0 and self.cap_w >= 0 and self.cap_s >= 0, (
        ...
        assert len(self.init_warehouse) == self.wh_lt + 1, (
            "Initial warehouse inventory must be the same length as the warehouse lead time"
        )
        assert len(self.init_store) == self.N, (
            "Initial store inventory must be the same length as the number of stores"
        )
        assert ringfence >= 0, (
            "The ringfenced amount for online demand needs to be positive"
        )

        if self.ringfence > 0:
            warnings.warn(
                "Ringfencing is not implemented yet. Please ignore this parameter."
            )
        ...
        for i in range(self.N):
            action_array.append(
                [1] + [self.cap_s if s != i else 1 for s in range(self.N)]
            )

        self.action_space = gym.spaces.MultiDiscrete(
            np.array(action_array), dtype=np.int16
        )
        # Set up state space
        self.observation_space = gym.spaces.Dict(
            {
                "warehouse": gym.spaces.Box(
                    low=np.zeros(self.wh_lt + 1),
                    high=np.ones(self.wh_lt + 1) * self.cap_w,
                    dtype=np.int32,
                ),
                "store": gym.spaces.Box(
                    low=np.zeros((self.N, self.st_lt + 1)),
                    high=np.ones((self.N, self.st_lt + 1)) * self.cap_s,
                    dtype=np.int32,
                ),
            }
        )
    ...
    def _RESET(self):
        ...
        self.store_demand = np.array(
            [
                self._gen_demand(
                    self.demand_distribution[i + 1], self.store_demand_params[i]
                )
                for i in range(self.N)
            ]
        ...
        self.wh_actions_log = np.zeros(
            T, dtype=np.int32
        )  # Logs the warehouse actions for the pipeline, i.e. how much got added
        self.st_actions_log = np.zeros(
            (self.N, T), dtype=np.int32
        )  # Logs the store actions for the pipeline
        self.st_actions_ts_log = np.zeros(
            (self.N, T), dtype=np.int32
        )  # Logs transhipment *incoming* actions (how many are added to the pipeline of store i)
    ...
    def move_state(self):
        ...
        for store in range(self.N):
            ...
            state["store"][store][0] = (
                self.I_o_H_st[store, t] + state["store"][store][1]
            )
    ...
    def fetch_state(self):
        ...
        if t == 0:
            ...
            self.wh_actions_log[: len(self.init_warehouse) - 1] = self.init_warehouse[
                1:
            ]
            self.st_actions_log[:, : len(self.init_store[0]) - 1] = np.array(
                self.init_store, dtype=np.int32
            )[:, 1:]
        else:
    ...
    def _STEP(self, action):
        ...
        starting_state = {
            "warehouse": self.state["warehouse"].copy(),
            "store": self.state["store"].copy(),
        }
        ...
        assert I_o_H_wh >= 0, (
            "You're orders to the stores ({}). have exceeded on hand warehouse ({}). Please implement an allocation policy in your ordering wrapper.".format(
                np.sum(orders[1:]), starting_state["warehouse"][0]
            )
        )
        ...
        if (
            self.ts_lt == 0
        ):  # Since transhipments can be zero lead-time we add to on hand if this is the case
        ...
        for store_idx, store in enumerate(I_o_H_st):
            if store < 0:
                dfw_request = self.np_random.binomial(
                    np.minimum(np.abs(store), np.maximum(I_o_H_wh, 0)), self.p
                )
        ...
        cost_wh = (
            ...
            if I_o_H_wh <= 0
            else I_o_H_wh * warehouse_holding
        )
        self.C_wh[t] = cost_wh
        ...
        for store_idx, store in enumerate(I_o_H_st):
            cost_st.append(
                np.abs(store) * penalty if store <= 0 else store * store_holding
            )
            cost_st[-1] += (
