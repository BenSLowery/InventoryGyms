...
class ts_TIE(Wrapper):
    ...
    def _get_inventory_position(self):
        ...
        """
        return {
            "Warehouse": self.unwrapped.state["warehouse"].sum(),
            "store": self.unwrapped.state["store"].sum(axis=1)
            + self._net_transhipments(),
        }
    ...
    def _allocate_stock(self, desired, avaialable):
        ...
        desired = [
            sp.poisson(self.unwrapped.online_demand_means[self.unwrapped.t]).ppf(
                self.unwrapped.cu / (self.unwrapped.cu + self.unwrapped.co_w)
            )
        ] + desired
    ...
    def _net_transhipments(self):
        ...
        for i in range(stores):
            s_sum[i] = sum(
                [
                    self.transhipment_arr[j, i] - self.transhipment_arr[i, j]
    ...
    def capped_base_stock_ordering_action(self, O_u_T, r=None):
        ...
        if total_store_orders > self.unwrapped.I_o_H_wh[t]:
            desired_store_orders = self._allocate_stock(
                desired_store_orders, self.unwrapped.I_o_H_wh[t]
            )
    ...
    def transhipment_action(self):
        ...
        )  # Inventory Position of stores for the transhipment lead-time

        # Get the demand over this lead time
        demand_stores = np.array(
            [
                self.unwrapped.store_demand_means[store][t]
                for store in range(self.unwrapped.N)
            ]
        )
        ...
        eq_func = (
            lambda d: d / demand_stores.sum() * IL.sum()
        )  # Lambda function to calculate equalised inventory
    ...
    def generate_action(self, ordering_type, ordering_action, transhipment=True):
        ...
        * transhipment: if a TIE transhipment is taking place this period
        ...
        if ordering_type == "EchBS":
            ...
            orders = RegularBaseStock.BaseStock(self.unwrapped).base_stock_action(
                ordering_action, True
            )
