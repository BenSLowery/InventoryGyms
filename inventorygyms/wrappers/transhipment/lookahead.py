...
class ts_la(Wrapper):
    ...
    def _calc_max_qs(self, d2_means):
        ...
        for idx, store_dist in enumerate(self.unwrapped.demand_distribution[1:]):
            if store_dist == "Poisson":
                ...
                store_mean = (
                    (d2_means[idx][0] * (1 - d2_means[idx][1])) / (d2_means[idx][1])
                    if d2_means[idx][1] != 0
                    else 0
                )
        ...
        initial_max_q = [
            np.floor((store_mean / sum(means)) * self.unwrapped.state["warehouse"][0])
            for store_mean in means
        ]
    ...
    def _lookahead(
        self, on_hand, k, d1_params, d2_params, max_order, distribution="Poisson"
    ):
        """
        ...
        if config in self.cached_configs:
            ...
            if distribution == "Poisson":
                exp, exp_first_stage, q = rust_lookahead.lookahead(
                    int(on_hand + k),
                    d1_params,
                    d2_params,
                    int(max_order),
                    cu=self.unwrapped.cu,
                    co=self.unwrapped.co_s,
                    cdfw=self.unwrapped.c_dfw,
                    p=self.unwrapped.p,
                )
    ...
    def generate_action(
        ...
        final_store_orders = [
            self._lookahead(
                ...
                d2_means[s],
                max_q[s],
                self.unwrapped.demand_distribution[s + 1],
            )[-1]
            for s in range(self.unwrapped.N)
        ]
        if warehouse_order_type == "EchBS":
            ...
            final_warehouse = max(
                warehouse_order
                - max(
                    self.unwrapped.state["warehouse"][0] - np.sum(final_store_orders), 0
                ),
                0,
            )
