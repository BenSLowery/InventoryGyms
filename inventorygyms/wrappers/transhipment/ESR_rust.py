...
class ts_ESR(Wrapper):
    ...
    def _two_period_expectation(
        self, idx, on_hand, k, d1_params, d2_params, distribution="Poisson"
    ):
        """
        ...
        if config in self.cached_configs:
            ...
            if self.r is None:
                if distribution == "Poisson":
                    ...
                    exp, exp_first_stage = rust_expectation.expectation(
                        int(on_hand + net_ts),
                        d1_params,
                        d2_params,
                        self.st_out[idx],
    ...
    def _allocate_stock(self, desired, avaialable):
        ...
        desired = [
            sp.poisson(self.unwrapped.online_demand_params[self.unwrapped.t]).ppf(
    ...
    def capped_base_stock_ordering_action(self, O_u_T, r=None):
        ...
        IP_st = self.unwrapped.state["store"].sum(axis=1) - np.array(
            [self._net_transhipments_sum(i, 0) for i in range(self.unwrapped.N)]
        )
